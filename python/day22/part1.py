...
AfterFallData = tuple[list[Brick], dict[Brick, list[Brick]], dict[Brick, list[Brick]]]
...
def get_support_data_after_fall(bricks: list[Brick]) -> AfterFallData:
    ...
    supporting_bricks_of = defaultdict(list)
    ...
    for i, brick in enumerate(bricks):
        ...
        for supporting_brick in supporting_bricks:
            ...
            supporting_bricks_of[brick].append(supporting_brick)

    return bricks, bricks_supported_by, supporting_bricks_of


def solve(input: str) -> int:
    bricks = parse(input)
    bricks, bricks_supported_by, supporting_bricks_of = get_support_data_after_fall(
        bricks
    )
    ...
    for brick in bricks:
        is_safe = all(
            len(supporting_bricks_of[b]) != 1 for b in bricks_supported_by[brick]
        )
        answer += is_safe
